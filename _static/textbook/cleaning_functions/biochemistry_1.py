...
def clean_doc_function_BIOCHEM(docs: list[Document], debug_logs: list[str] | None = None) -> (list[Document]):
    def strip_pages(docs: list[Document], page_list: list[any], page_metadatas: list[any]) -> tuple[list[any], list[any]]:
        ...
        def add_paragraph(doc_metadata: dict, current_paragraph: list[str], paragraphs: list[any], paragraph_metadatas: list[dict]):
        ...
        for i, doc in enumerate(docs):
            ...
            for i, line in enumerate(lines[:-1]):  # exclude last line (page number/description)
                ...
                if (len(line) < CHAR_THRESHOLD):
                    ...
                    add_paragraph(doc.metadata, current_paragraph, paragraphs, paragraph_metadatas)
                    current_paragraph = []
    ...
    def combine_paragraphs(page_list: list[any], page_metadatas: list[any]) -> tuple[list[str], list[dict]]:
        ...
        def paragraph_validate(paragraph: list[str]):
            ...
            if is_text_question(combined_text):
                if debug_logs is not None:
                    print("rejected due to being a question:", combined_text)
                    debug_logs.append(f"PARAGRAPH REJECTED DUE TO BEING A QUESTION. Text: '{combined_text}'")
                return False
        ...
        for i, page in enumerate(page_list):
            for j, paragraph in enumerate(page):
                if not paragraph_validate(paragraph):
                    if debug_logs is not None:
    ...
    for i in range(len(paragraphs_combined)):
        ...
        if len(combined_text) < 80:
            if debug_logs is not None:
                debug_logs.append(f"POST COMBINE: DISCARDED PARAGRAPH DUE TO LENGTH < 80: '{combined_text}'")
